  ...
  const optionscolumnchart: any = {
    ...
    useEffect(() => {
      if(data !== null && seriescolumnchart[0].data?.length === 0){
        setSeriescolumnchart([{ name: '', color: secondary, data: data.map((item: any) => item.staked) }]);
      }
    },[data]);
    return (
          ...
          >
            ...
            <Typography variant="subtitle2" color="textSecondary">
              ...
              <Typography variant="h4">
                {data !== null ? Math.round(data[data.length-1].staked).toLocaleString("en-US") : 0}
              </Typography>
              <Stack direction="row" spacing={1} mt={1} alignItems="center">
              <Avatar sx={
                    // percentage difference from previous month
                    data !== null ? (
                      data[data.length-1].staked > data[data.length-2].staked ? { bgcolor: successlight, width: 24, height: 24 } : 
                      data[data.length-1].staked < data[data.length-2].staked ? { bgcolor: errorlight, width: 24, height: 24 } : { bgcolor: successlight, width: 24, height: 24 }
                    ) : { bgcolor: successlight, width: 24, height: 24 }
                  }>
                  {
                    // percentage difference from previous month
                    data !== null ? (
                      data[data.length-1].staked > data[data.length-2].staked ? <IconArrowUpLeft width={18} color="#39B69A" /> : 
                      data[data.length-1].staked < data[data.length-2].staked ? <IconArrowDownRight width={18} color="#FA896B" /> : <IconEqual width={18} color="#39B69A" />
